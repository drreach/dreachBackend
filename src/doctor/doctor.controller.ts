...
import { UpdateSheduleDto, bookAppointmentDTO, hybridBookAppointmentDTO } from 'src/user/dto/user.dto';
import { formatISO } from 'date-fns';
import { GoogleMeetService } from 'src/google.service';

@Controller('doctor')
export class DoctorController {
  constructor(private readonly doctorService: DoctorService,private readonly meetService:GoogleMeetService) {}
  ...
  async getDoctorProfile(@Query() dto:{username:string,userId:string,date:string}) {
    // console.log(username,userId)
    
    return this.doctorService.sheduleThings(dto.username,dto.userId);
  } 
  
  @Get('getdoctorProfilebyVideo')
  async getDoctorProfilebyVideo(@Query() dto:{username:string,userId:string,date:string,slot:string}) {
    // console.log(username,userId)
    
    return this.doctorService.getSlotsByVideoConsult(dto.username,dto.userId,dto.date,dto.slot);
  }
  
  @Get('getdoctorProfilebyHome')
  async getDoctorProfilebyHome(@Query() dto:{username:string,userId:string}) {
    // console.log(username,userId)
  ...
  async checkAvailabilty(@Body() dto:{doctorId:string,date:string,slot:string,mode:string}){
    return this.doctorService.checkDoctorAvailability(dto.doctorId,dto.date,dto.slot,dto.mode)
  } 
  
  @Post('hybridcheckDoctorAvailability')
  async hybridCheckAvailabilty(@Body() dto:{homeVisitDoctorId:string,h_apptDate:string,h_slotTime:string,videoDoctorId:string,v_apptDate:string,v_slotTime:string}){
 ...
 @Post("hybridBookAppointment")
  ...
  async getPatientsByIdByDoctor(@Query() dto:{pid:string,doctorId:string}) {
  ...
  async addMedicalRecord(@UploadedFile() file: Express.Multer.File,@Body() dto:{patientsId:string,doctorId:string,description:string}) {
    console.log(file,dto);
    return this.doctorService.addMedicalRecord(dto.patientsId,dto.doctorId,file,dto.description);
  }
  @Post('addDocuments')
  @UseInterceptors(FileInterceptor('document'))
  async addDocuments(@UploadedFile() file: Express.Multer.File,@Body("doctorId") doctorId:string ) {
