...
export class UserService {
  ...
  async sendOTP(phone: string) {
    ...
    const res = await fetch(
      `https://www.fast2sms.com/dev/bulkV2?authorization=cnRjDofkWNu30eJm9PlYwGCybhLxpVXEtT2aAzM78vI5ZdUS6Fib3emyuDP20CZtB8XRAj4KxwLsnalN&route=otp&variables_values=${otp}&flash=0&numbers=${phone}`,
      {
        method: 'GET',
        headers: {
    ...
    // await this.utils.sendOTP(phone, otp);
  ...
  async registerUser(dto: {
    ...
    try {
      ...
      const otp = await this.sendOTP(dto.phone);
  ...
  async verifyUserRegistration(phone: string, otp: number) {
    try {
      const cacheOtp: {
        otp: number;
        firstName: string;
        lastName: string;
        password: string;
      } = await this.cacheService.get(phone);
      ...
      const hashedPassword = await bcrypt.hash(cacheOtp.password, salt);

      const createdUser = await this.prisma.user.create({
        data: {
          phone: phone,
          Fname: cacheOtp.firstName,
          Lname: cacheOtp.lastName,
          role: 'NORMAL',
